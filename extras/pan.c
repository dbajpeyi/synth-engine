...
void dataCallback(ma_device* pDevice, void* pOutput, const void* pInput, ma_uint32 frameCount) {
	...
	void* buffArr = *(buffer) + start * ma_get_bytes_per_frame(pDecoder->outputFormat, pDecoder->outputChannels);
...
int main(int argc, char** argv) {
	...
	outputBuffer = malloc(buffLength * ma_get_bytes_per_frame(decoder.outputFormat, decoder.outputChannels));

	user_data userData = {&decoder, &outputBuffer, &trackingIndex, &hasEnded, &peakValue};
