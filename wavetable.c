...
float sawtooth(float phase) { return fmod((phase + PI) / PI, 2) - 1; }
...
float lookupTable(float *waveTable, float index, int tableLength) {
  ...
  return nearestHighWeight * waveTable[nearestHigh] +
...
void writeCustomWaveform(float *waveTable, int tableLength) {
  ...
  if (ma_encoder_write_pcm_frames(&wEncoder, waveTable,
                                  (int)(tableLength / NUM_CHANNELS),
                                  NULL) != MA_SUCCESS) {
    printf("Failed to initialize output file.\n");
    exit(0);
  }
